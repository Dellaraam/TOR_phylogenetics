...
```{r}
RICTORMetamonadaPlot <- ggplot(filter(RictorDataPlot, `Super Group` == "Metamonada"), aes(x = sca, y = scd, color = C.score, label = Organism_Name))+
  geom_point()+
  geom_text_repel(data = filter(RictorDataPlot, `Super Group` == "Metamonada"),
                  direction = "y",
...
RICTORDiscobaPlot <- ggplot(filter(RictorDataPlot, `Super Group` == "Discoba"), aes(x = sca, y = scd, color = C.score, label = Organism_Name))+
  geom_point()+
  geom_text_repel(data = filter(RictorDataPlot, `Super Group` == "Discoba"),
                  direction = "y",
                  min.segment.length = 0.5,
                  force = 10,
                  max.time = 10,
