...
Rhizaria_Busco <- read.csv("~/TOR_phylogenetics/GitHub_CSV/Rhizaria_BUSCO_Sheet1.csv")
...
Ciliates <- read_tsv("C:/Users/kajoh/Documents/GitHub/TOR_phylogenetics/Checking_CSVs/Ciliates_Names.tsv")
```
...
RAPTORRhizaria <- RAPTORRhizariaRaw %>% filter(sca >= RSDScoreA & scd >= RSDScoreD)
...
TORRhizaria <- merge(LST8Rhizaria, Rhizaria_Busco[c("C.score", "Frag.score", "Accn")], by = "Accn")
```
...
RICTORAlveolataPlotCiliate <- ggplot(RICTORAlveolata, aes(x = sca, y = scd, color = C.score, label = Organism.Name))+
  geom_point()+
  geom_text_repel(data = subset(RICTORAlveolata, Ciliate_Check == TRUE),
                  direction = "y",
                  min.segment.length = 0.5,
                  force = 10,
                  max.time = 10,
