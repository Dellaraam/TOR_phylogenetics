...
Ciliates <- read_tsv("C:/Users/kajoh/Documents/GitHub/TOR_phylogenetics/Checking_CSVs/Ciliates_Names.tsv", show_col_types = FALSE)
...
#length(SIN1Rhizaria)
test1 <- mainCombine("Alveolata", Alveolata_Names, RICTORAlveolata, SIN1Alveolata, RAPTORAlveolata, LST8Alveolata, TORAlveolata, D1Alveolata)
test1 <- test1[!duplicated(test1$Organism_Taxonomic_ID),]
test1 <- merge(test1, Alveolata_Busco[c("C.score","Frag.score", "Accn")], by = "Accn")

#test1



test1 <- parasiteCheck(test1, Apicomplexa, Perkinsozoa)
test1 <- Check_Ciliates(test1, Ciliates)
test1
...
# Rhizaria (Future)
D1Rhizaria <- merge(D1Rhizaria, Rhizaria_Names[c("Organism_Taxonomic_ID", "Organism.Name", "Accn")], by = "Accn")
RICTORRhizaria <- merge(RICTORRhizaria, Rhizaria_Names[c("Organism_Taxonomic_ID", "Organism.Name", "Accn")], by = "Accn")
RAPTORRhizaria <- merge(RAPTORRhizaria, Rhizaria_Names[c("Organism_Taxonomic_ID", "Organism.Name", "Accn")], by = "Accn")
LST8Rhizaria <- merge(LST8Rhizaria, Rhizaria_Names[c("Organism_Taxonomic_ID", "Organism.Name", "Accn")], by = "Accn")
TORRhizaria <- merge(TORRhizaria, Rhizaria_Names[c("Organism_Taxonomic_ID", "Organism.Name", "Accn")], by = "Accn")
```
...
```{r}
# create Combined tables that have the information 
...
CombinedAlveolata <- CombinedAlveolata[!duplicated(CombinedAlveolata$Organism_Taxonomic_ID),]
...
CombinedAlveolata <- CombinedAlveolata %>% add_column(SubClade = NA, .after = "Group")
#Begin the iterative loops
for (i in 1:nrow(CombinedAlveolata)){
  print(CombinedAlveolata[i, "Organism_Taxonomic_ID"])
  print("Apicomplexa Loop")
  for(j in 1:nrow(Apicomplexa)) {
    if (CombinedAlveolata[i, "Organism_Taxonomic_ID"] == Apicomplexa[j, "Organism_Taxonomic_ID"]){
      #print(Apicomplexa[j, "Organism_Taxonomic_ID"])
      CombinedAlveolata$SubClade[i] <- "Apicomplexa"
    }else{next}
  }
  print("Chromerida Loop")
  for(h in 1:nrow(Chromerida)){
    if (CombinedAlveolata[i, "Organism_Taxonomic_ID"] == Chromerida[h, "Organism_Taxonomic_ID"]){
      CombinedAlveolata$SubClade[i] <- "Chromerida"
    }else{next}
  }
  print("Perkinsozoa Loop")
  for(k in 1:nrow(Perkinsozoa)){
    if (CombinedAlveolata[i, "Organism_Taxonomic_ID"] == Perkinsozoa[k, "Organism_Taxonomic_ID"]){
      CombinedAlveolata$SubClade[i] <- "Perkinsozoa"
    }else{next}
  }
  
  for(m in 1:nrow(Dinozoa)){
    if (CombinedAlveolata[i, "Organism_Taxonomic_ID"] == Dinozoa[m, "Organism_Taxonomic_ID"]){
      CombinedAlveolata$SubClade[i] <- "Dinozoa"
    }else{next}
  }
  
  for(n in 1:nrow(Ciliates)){
    if (CombinedAlveolata[i, "Organism_Taxonomic_ID"] == Ciliates[n, "Organism_Taxonomic_ID"]){
      CombinedAlveolata$SubClade[i] <- "Ciliates"
    }else{next}
  }
...
CombinedAlveolata %>% arrange(desc(Organism_Taxonomic_ID))
```

```{r}
CombinedStramenopiles <- mainCombine("Stramenopile", Stramenopiles_Names, cleanedRICTORStramenopiles, cleanedSIN1Stramenopiles, cleanedRAPTORStramenopiles, cleanedLST8Stramenopiles, cleanedTORStramenopiles, cleanedD1Stramenopiles)
CombinedStramenopiles <- CombinedStramenopiles[!duplicated(CombinedStramenopiles$Organism_Taxonomic_ID), ]
CombinedStramenopiles <- merge(CombinedStramenopiles, Stramenopiles_Busco[c("C.score","Frag.score", "Accn")], by = "Accn")
CombinedStramenopiles <- CombinedStramenopiles %>% add_column(SubClade = NA, .after = "Group")
#Iterative loops
for(a in 1:nrow(CombinedStramenopiles)){
  
  for(b in 1:nrow(Bigyra)){
    if(CombinedStramenopiles[a, "Organism_Taxonomic_ID"] == Bigyra[b, "Organism_Taxonomic_ID"]){
      CombinedStramenopiles$SubClade[a] <- "Bigyra"
    }
  }
  
  for(c in 1:nrow(Ochrophyta)){
    if(CombinedStramenopiles[a, "Organism_Taxonomic_ID"] == Ochrophyta[c, "Organism_Taxonomic_ID"]){
      CombinedStramenopiles$SubClade[a] <- "Ochrophyta"
    }
  }
  
  for(d in 1:nrow(Oomycota)){
    if(CombinedStramenopiles[a, "Organism_Taxonomic_ID"] == Oomycota[d, "Organism_Taxonomic_ID"]){
      CombinedStramenopiles$SubClade[a] <- "Oomycota"
    }
  }
}
...
CombinedRhizaria <- mainCombine("Rhizaria", Rhizaria_Names, cleanedRICTORRhizaria, SIN1Rhizaria, cleanedRAPTORRhizaria, cleanedLST8Rhizaria, cleanedTORRhizaria, cleanedD1Rhizaria)
CombinedRhizaria <- CombinedRhizaria[!duplicated(CombinedRhizaria$Organism_Taxonomic_ID)]
CombinedRhizaria <- merge(CombinedRhizaria, Rhizaria_Busco[c("C.score","Frag.score", "Accn")], by = "Accn")
CombinedRhizaria <- CombinedRhizaria %>% add_column(SubClade = NA, .after = "Group")
```
...
RICTORAlveolataPlotCiliate <- ggplot(RICTORAlveolata, aes(x = sca, y = scd, color = C.score, label = Organism.Name))+
  geom_point()+
